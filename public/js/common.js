...
let cameraId = '';
let micId = '';
function login(roomId, sdkAppId, userId, userSign, shareUserSign) {
    presetting.login(false, roomId, sdkAppId, userId, userSign, shareUserSign, options => {
        rtc = new RtcClient(options);
        join();
    });
    presetting.login(true, roomId, sdkAppId, userId, userSign, shareUserSign, options => {
