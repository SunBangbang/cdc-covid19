<template>
  ...
  >
    ...
    >
      ...
      <el-form-item
        v-show="dataForm.receiverType == 1"
        size="mini"
        :label="$t('notice.selectDept')"
      >
        <el-tree
          :data="deptList"
    ...
    <template slot="footer">
      ...
      <el-button type="danger" @click="dataFormSubmitHandle(0)">{{
        $t("notice.draft")
      }}</el-button>
      <el-button type="primary" @click="dataFormSubmitHandle(1)">{{
        $t("notice.release")
      }}</el-button>
    </template>
  </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.$http
        .get(`/sys/notice/${this.dataForm.id}`)
        .then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg);
          }
          ...
          if (this.dataForm.receiverType === 1) {
            this.$refs.deptListTree.setCheckedKeys(
              res.data.receiverTypeIds.split(",")
            );
          }
        })
        .catch(() => {});
    ...
    dataFormSubmitHandle: debounce(
      function(status) {
        this.$refs["dataForm"].validate(valid => {
          ...
          // 接受者为部门
          if (this.dataForm.receiverType === 1) {
            this.dataForm.receiverTypeIds = this.$refs.deptListTree
              .getCheckedKeys()
              .join(",");
