<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    ...
    <template slot="footer">
      <el-button type="primary" @click="dataFormSubmitHandle()" size="mini">{{ $t('confirm') }}</el-button>
...
export default {
  ...
  methods: {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/biz/dict/type/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/biz/dict/type', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
