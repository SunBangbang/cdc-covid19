...
const conversationModules = {
  ...
  getters: {
    toAccount: state => {
      ...
      console.log('*********** toAccount:', state.currentConversation.type, '  ', state.currentConversation)
    ...
    toNick: state => {
      ...
      switch (state.currentConversation.type) {
        case 'C2C':
          return state.currentConversation.userProfile.nick
        case 'GROUP':
  ...
  mutations: {
    ...
    pushCurrentMessageList(state, data) {
      ...
      if (Array.isArray(data)) {
        ...
        let tmpMessageList = [...state.currentMessageList, ...result]
        filterCallingMessage(tmpMessageList, state.currentMessageList = [])
      } else if (data.conversationID === state.currentConversation.conversationID) {
        let tmpMessageList = [...state.currentMessageList, data]
        filterCallingMessage(tmpMessageList, state.currentMessageList =[])
      }
    },
    /**
  ...
  actions: {
    ...
    getMessageList(context, conversationID) {
      ...
      tim.getMessageList({ conversationID, nextReqMessageID, count: 15 }).then(imReponse => {
        ...
        if(!context.state.isCompleted && !imReponse.data.nextReqMessageID){
          context.state.isCompleted = true
        }
        // 更新当前消息列表，从头部插入
        let tmpMessageList = [...imReponse.data.messageList, ...currentMessageList]
        //点击多次查看消息可能重复
        filterCallingMessage(tmpMessageList, context.state.currentMessageList =[] )
      })
    },
    /**
     * 切换会话
