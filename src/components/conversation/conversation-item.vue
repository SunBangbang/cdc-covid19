<template>
    ...
    >
      ...
      <div class="warp">
        <avatar :src="avatar" :type="conversation.type" v-if="conversation.type !== 'GROUP'"/>
        <avue-avatar :src="icon" :type="conversation.type" v-else size="large" style="margin-right: 10px"></avue-avatar>
        <div class="content">
          <div class="row-1">
            <div class="name">
              <div class="text-ellipsis" style="font-weight: bold">
          ...
          <div class="row-2">
            <div class="summary">
              <div v-if="conversation.lastMessage" class="text-ellipsis" style="color: #a6a9ad;text-overflow: ellipsis; overflow: hidden; width: 100%">
                <div class="remind" style="color:red;" v-if="hasMessageAtMe">[有人提到我]</div>
                <div class="text" :title="conversation.lastMessage.messageForShow" style="height: 13px">
                  {{messageForShow}}
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    messageForShow() {
      ...
      }else if (this.conversation.lastMessage.payload && this.conversation.lastMessage.payload.second){
        return '[语音消息]'
      }else if (this.conversation.lastMessage.payload && this.conversation.lastMessage.payload.data){
        return '[视频通话]'
      }
