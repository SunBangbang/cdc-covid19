...
<!--          />-->
        ...
        <div style="margin-top: 5px;font-size: 20px;color: black;" :title="groupProfile.name" v-if="!showEditName">
...
<!--      </div>-->
      <div class="info-item">
        <div class="label" :class="{'active' : active}" style="margin-bottom: 5px;">全体禁言</div>
        <el-switch
      ...
      <div>
        <el-button type="text" @click="dialogVisible = true">群管理</el-button>
      </div>
      <el-dialog
              title="群管理"
...
import { groupErrorMap } from '@/utils/common'
  ...
  methods: {
    ...
    quitGroup() {
      ...
      .catch(error => {
          this.$store.commit('showMessage', {
            type: 'error',
            message: this.groupErrorMap[error.code] || error.message
          })
        })
    },
    dismissGroup() {
