<template>
  <div>
    <el-form
      ...
      :disabled="disabled"
    >
    <div>{{getData(columns)}}</div>
      <el-row :gutter="gutter">
        <el-col  l-col v-for="(item, index) in columns" :key="index" :span="item.span">
        ...
        <!-- <div>{{item}}</div> -->
          ...
          <!--  标题  -->
          <div v-if="item.type === 'title' && item.coreItem">
            <h3>{{ item.params.html }}</h3>
          </div>

          <!-- 表述信息 -->
          <p
            v-else-if="item.type === 'description' && item.coreItem"
            class="desc-text"
          >
            {{ item.params.html }}
          </p>
          ...
          <!--  表格  -->
          <div
            v-else-if="item.type === 'table' && item.coreItem"
            class="form-tab"
            :id="'label_' + item.prop"
          >
            <avue-crud
          ...
          <!-- group -->
          <base-form-group-core
            v-else-if="item.type === 'group' && item.display && item.coreItem"
            :item="item"
            :form="form"
          ...
          <!-- dynamic -->
          <base-form-dynamic-core
            v-else-if="item.type === 'dynamic' && item.display && item.coreItem"
            :item="item"
            :columns="columns"
            :form="form"
            :disabled="disabled"
          ...
          <el-form-item
            v-else-if="item.display && item.coreItem"
            :rules="getRule(item)"
            :id="'label_' + item.prop"
            :prop="item.prop"
          >
...
import BaseFormGroupCore from "@/components/form-design/form-json/base-form-group-core";
import BaseFormDynamicCore from "./base-form-dynamic-core";
...
export default {
  ...
  methods: {
    ...
    handleClick(row, form, item) {
      ...
      // console.log(this.$refs['confirm-table-' + item.prop]);
