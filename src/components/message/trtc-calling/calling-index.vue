          ...
          <div class="call" v-show="dialling || calling">
            ...
            <div v-show=" callType === TRTCCalling.CALL_TYPE.VIDEO_CALL && calling">
              <div class="small-group" id="small-group">
                ...
                <div class="video-box" v-for="userId in callingUserList" :id="`video-${userId}`"
                     :key="`video-${userId}`">
                </div>
              </div>
            </div>
            <div v-show="callType === TRTCCalling.CALL_TYPE.AUDIO_CALL && calling" class="audio-box">
              ...
              <div v-for="item in invitedUserInfo " class="aduio-call" :key="`video-${item.userID}`">
                ...
                <p style="text-align: center">
                  ...
                  <i v-if="item.isInvitedMicOn === true || item.isInvitedMicOn == undefined"
                     class="el-icon-microphone micr-icon" style="color: #006FFF;"></i>
                  <i v-else class="el-icon-turn-off-microphone micr-icon"></i>
                </p>
              </div>
            </div>
            ...
            <div class="buttons" v-if="callType === TRTCCalling.CALL_TYPE.VIDEO_CALL">
              <div class="small1" @click="small"></div>
...
export default {
  ...
  computed: {
    ...mapGetters(['toAccount', 'toNick', 'currentConversationType']),
    ...mapState({
  ...
  methods: {
    ...
    initListener() {
      console.log('**************** calling index init listener.........................')
      // sdk内部发生了错误
    ...
    removeListener() {
      console.log('**************** calling index remove listener.........................')
    ...
    async handleNewInvitationReceived(payload) {
      console.log('.......... handleNewInvitationReceived:', payload)
      ...
      if(sponsor) {
        getUserRealName(sponsor).then(res => {
          if(res.data.code === 0) {
            this.sponsorName = res.data.data.realName
          }
        })
      }
    ...
    changeState(state, boolean) {
      ...
      if (this.dialling || this.calling || this.isDialled) {
        this.dialogVisible = true
        this.$emit("showMainContent", false)
        this.$store.commit('updateChatDialog', false)
      } else {
        this.dialogVisible = false
        this.$emit("showMainContent", true)
        if (this.currentConversation && this.currentConversation.conversationID) {
          this.$store.commit('updateChatDialog', true)
        }
      }
      console.log('************calling index change state:', state, ' ', boolean)
    ...
    audioCalling() { // 发起通话
      ...
      console.log('audio calling:', this.callingType)
...
.accept, .refuse, .videoOn, .videoOff, .micOn, .micOff, .small1
  height 50px
  width 50px
...
.small1
  background center no-repeat url("../../assets/image/small.png")
  background-size 70%
